...
export function InitializeProgressButton({ newWordsCount }: { newWordsCount: number }) {
  ...
  return (
    <div className="rounded-lg border border-amber-200 bg-amber-50 p-5 dark:border-amber-900 dark:bg-amber-950">
      <h2 className="text-base font-semibold text-amber-950 dark:text-amber-100">Add {newWordsCount} words to your review rotation</h2>
      ...
      {message ? <p className="mt-3 text-sm text-amber-800 dark:text-amber-200">{message}</p> : null}
    </div>
  );
}
